...
private:
  static std::vector<float> emptyV;
  static std::vector<std::vector<std::vector<float> > > emptyVVV;
public:
  virtual ~DataChunk() {}
  virtual size_t get_data_dim(void) const { return 0; }
  virtual std::vector<float> const & get_1d() const { /* NOTIMPL */ return emptyV; };
  virtual std::vector<std::vector<std::vector<float>>> const &get_3d() const { return emptyVVV; };
